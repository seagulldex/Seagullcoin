...
async function getAccountSequence(address) {
  try {
    ...
    // Fetch account info for the specified address
    const accountInfo = await api.getAccountInfo(address);

    // Return the sequence number
    return accountInfo.sequence;
  } catch (err) {
    console.error('[ERROR] Failed to get account sequence:', err);
    throw new Error('Unable to retrieve account sequence');
  } finally {
    await api.disconnect();
  }
}

async function lockSeagullCoinForStaking(walletAddress, amount, duration) {
  try {
    await api.connect(); // Connect to the XRP Ledger network

    // Staking pool wallet address
    const stakingWallet = 'rHN78EpNHLDtY6whT89WsZ6mMoTm9XPi5U'; 

    // Create the payment transaction for staking
    const payment = {
      TransactionType: 'Payment',
      Account: walletAddress, // The user's wallet
      Destination: stakingWallet, // The staking wallet
      Amount: amount, // Amount to stake
      Fee: '12', // Transaction fee (adjust as needed)
      Sequence: await getAccountSequence(walletAddress) // Get account sequence for the user
    };

    // Prepare and sign the transaction
    const preparedTx = await api.prepareTransaction(payment);
    const signedTx = api.sign(preparedTx.txJSON, walletAddress.privateKey);
    ...
    // Save the staking data in the database
    const startTime = Date.now();
    const endTime = startTime + (duration * 24 * 60 * 60 * 1000); // Calculate end time in ms
    db.run(
      `INSERT INTO staking (walletAddress, amount, duration, startTime, endTime, status, stakingTxId)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [walletAddress, amount, duration, startTime, endTime, 'active', result.txId],
      function (err) {
        if (err) {
