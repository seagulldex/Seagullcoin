...
import XummSdk from 'xumm-sdk';  // Correct import
...
// Initialize XummSdk directly
const xumm = XummSdk(process.env.XUMM_API_KEY, process.env.XUMM_API_SECRET);
...
app.get('/nfts', async (req, res) => {
  ...
  // Filter out non-SeagullCoin NFTs
  const seagullCoinNFTs = nfts.filter(nft => nft.token === "SeagullCoin");

  res.json(seagullCoinNFTs);
});

app.post('/transfer-nft-metadata', async (req, res) => {
  const { nftId, fromWallet, toWallet } = req.body;

  // Ensure the NFT is SeagullCoin-based
  const nft = await getNFTDetails(nftId);
  if (nft.token !== "SeagullCoin") {
    return res.status(400).json({ error: 'Only SeagullCoin NFTs can transfer metadata' });
  }
...
app.post('/cancel-xrp-offers', async (req, res) => {
  const { nftId, offerCurrency } = req.body;

  if (offerCurrency === "XRP") {
    // Reject and cancel the offer
    await cancelXRPOffer(nftId);
    return res.status(200).json({ status: 'XRP offer cancelled' });
  }

  // Proceed if the offer is not in XRP
  res.status(400).json({ error: 'Only SeagullCoin is allowed for offers' });
});

app.post('/approve-nft', async (req, res) => {
  const { nftId, token } = req.body;

  // Check if the NFT is using SeagullCoin
  if (token !== "SeagullCoin") {
    return res.status(400).json({ error: 'NFTs must be minted using SeagullCoin' });
  }
